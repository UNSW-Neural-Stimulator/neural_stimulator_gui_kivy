...
live_update_references = {
    #gotta add dc mode -> Yuden Sae-Ueng 23/06/2022
    #gotta add ramp up time
...
def get_stimulator_input():
    ...
    dc = settings['dc_toggle'] != 'down'
    ramp_up_time = int(settings['ramp_up_time_input']) if settings[
                                                                           'ramp_up_time_input'] != "" else 0
    ...
    return settings, int(burstmode), int(burstperiod), int(burstduration), int(dutycycle), int(interburst), int(
        anodic), int(dc), int(ramp_up_time), int(phase_1_current), int(phase_2_current), int(interphase), int(phasetime1), int(phasetime2), int(
...
def get_squarewave_plot():
    ...
    dutycycle, interburst, anodic, dc, ramp_up_time,phase_1_current, \
...
def start_stimulation(button, state):
    ...
    if state == 'down':
        ...
        #TODO 26/06/2022 Yuden Sae-Ueng -> confirm ramp up time value limits
        ...
        if dc < 0 or dc > 1:
            error_messages.append("dc mode invalid value: range True or False")
        ...
        if len(error_messages) > 0:
            ...
            print(f"dc mode is: {dc}")
            print(f"ramp up time is: {ramp_up_time}")
...
class NeuroStimApp(App):
    ...
    def get_graph_variables(self):
        ...
        return {
            ...
            'ramp_up_time_input': digit_clean(self.get_components('ramp_up_time_input')),
            'anodic_toggle': self.get_components('anodic_toggle').state,
            'dc_toggle': self.get_components('dc_toggle').state,
    ...
    def get_components(self, id):
        ...
        if id == 'ramp_up_time_input':
            return self.get_components('stimulation_tabs').ramp_up_time_input
        ...
        if id == 'ac_dc_toggle':
            return self.get_components('stimulation_tabs').ac_dc_toggle
        if id == 'ac_toggle':
            return self.get_components('ac_dc_toggle').ac
        if id == 'dc_toggle':
            return self.get_components('ac_dc_toggle').dc
